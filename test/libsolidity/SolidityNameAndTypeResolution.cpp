...
BOOST_AUTO_TEST_CASE(non_payable_constructor)
{
	...
	CHECK_ERROR(text, TypeError, "Member \"value\" not found or not visible after argument-dependent lookup in function () public returns (contract C) - did you forget the \"payable\" modifier?");
}
...
BOOST_AUTO_TEST_CASE(function_type_arrays)
{
	char const* text = R"(
		contract C {
			...
			function f() public {
				function(uint) public returns (uint)[10] memory a;
				function(uint) public returns (uint)[10] storage b = y;
...
BOOST_AUTO_TEST_CASE(warn_function_type_return_parameters_with_names)
{
	char const* text = R"(
		contract C {
			function(uint) public returns (bool ret) f;
		}
	)";
...
{
	CHECK_WARNING("contract C { function f() public { selfdestruct; } }", "Statement has no effect.");
	CHECK_WARNING("contract C { function f() public { assert; } }", "Statement has no effect.");
	CHECK_WARNING("contract C { function f() public { require; } }", "Statement has no effect.");
	CHECK_WARNING("contract C { function f() public { suicide; } }", "Statement has no effect.");
}
...
BOOST_AUTO_TEST_CASE(shadowing_builtins_with_parameters)
{
	char const* text = R"(
		contract C {
			function f(uint require) public {
				require = 2;
			}
		}
	)";
...
BOOST_AUTO_TEST_CASE(shadowing_builtins_with_return_parameters)
{
	char const* text = R"(
		contract C {
			function f() public returns (uint require) {
				require = 2;
			}
		}
	)";
...
BOOST_AUTO_TEST_CASE(too_large_arrays_for_calldata)
{
	...
	text = R"(
		contract C {
			function f(uint[85678901234] a) public {
			}
		}
	)";
	CHECK_ERROR(text, TypeError, "Array is too large to be encoded.");
